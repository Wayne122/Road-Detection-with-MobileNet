...
class MobileNetSkipAdd(nn.Module):
    def __init__(self, output_size, pretrained=False, dorn=False):
...
class MobileNetSkipAdd_DORN(nn.Module):

    def __init__(self, output_size, intervals=80):
        super(MobileNetSkipAdd_DORN, self).__init__()
        assert os.path.isfile('./results/mobilenet-nnconv5dw-skipadd-pruned.pth.tar'), \
            "=> no model found at '{}'".format('./results/mobilenet-nnconv5dw-skipadd-pruned.pth.tar')
        print("=> loading model '{}'".format('./results/mobilenet-nnconv5dw-skipadd-pruned.pth.tar'))
        ...
        if type(checkpoint) is dict:
            ...
            self.model.dorn =True
        self.classifier = nn.Conv2d(1, intervals, 1)
...
if __name__ == "__main__":
    module = MobileNetSkipConcat(224, pretrained=False)
    print('finished')